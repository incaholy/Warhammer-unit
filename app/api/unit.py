...
def list_units(
    ...
    owned_by: UUID | None = Depends(get_owned_by),
) -> Page[Unit_Read]:
    ...
    # filtering and pagination have to happen on the same side. The `owned` query
    # parameter is declared by `get_owned_by`, which also resolves the caller.
...
def unit_facets(
    ...
    total, by_faction = service.faction_facets(subfaction_id=subfaction_id, q=q, owned_by=owned_by)
